...
class UserSolution
{
	...
	{
//		boolean exist = false;
//		for (pair<Integer> auto : adj0[mStartAirport][mStartTime])
//		{
//			int endPoint = auto.first;
//			int	travelTime = auto.second;
//			if (endPoint == mEndAirport)
//			{
//				exist = true;
//				if (mTravelTime < travelTime)
//					auto.second = mTravelTime;
//				break;
//			}
//		}
//		if (exist == false)
	...
	public int minTravelTime(int mStartAirport, int mEndAirport, int mStartTime)
	{
		...
		while (!pq.isEmpty())
		{
			...
			if (d0[current][arrival] != totalCost)
				continue ;
			if (current == mEndAirport) // for (auto &[next, cost] :  adj0[cur][arrival]) {}
				return totalCost;
...
//			for (pair<Integer> auto : adj0[current][arrival])
//			{
//				int	next = auto.first;
//				int	cost = auto.second;
//				int	next_t = (arrival  + cost) % 24;
//				if (d0[next][next_t] > d0[current][arrival] + cost)
//				{
//					d0[next][next_t] = d0[current][arrival] + cost;
//					pq.add(new pair(d0[next][next_t], new pair<>(next, next_t)));
//				}
//			}
		}
		return -1;
